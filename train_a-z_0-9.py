...
import model
from dataset.dataset import CHDataset
...
writer = SummaryWriter("logs/train_a-z_0-9")
...
data = CHDataset("data", train=True, transform=transform)
...
# 创建一个网络
train_net = model.Net(isAtoZ=True).to(device)
# 优化器
...
for epoch in range(epochs):
    ...
    for data in train_loader:
        ...
        loss.backward()
        nn.utils.clip_grad_norm_(train_net.parameters(), max_norm=3, norm_type=2)
        optimizer_sgd.step()
        total_train_step += 1
    ...
    total_test_step += 1
    torch.save(train_net, f"output/AZ/train_net_{epoch + 1}.pth")
